...
def parser_arguments():
    ...
    parser.add_argument(
        "--perlayer_kv",
        help="v7: use per-layer KV cache inputs instead of shared tensor",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--kv_inplace",
        help="v11: KV cache in-place update, mask shape = kv_cache_length (no +seq_len)",
        action="store_true",
        default=False,
    )
...
def export_onnx(
    ...
    perlayer_kv: bool = False,
    kv_inplace: bool = False,
):
    if device_str == "npu":
        import torch_npu
    if dtype == "float16":
    ...
    if perlayer_kv:
        # v7: per-layer KV cache
        input_names = ["input_ids", "attention_mask", "position_ids"] + \
                      [f"past_kv_{i}" for i in range(num_hidden_layers)]
        output_names = ["logits"] + \
                       [f"out_kv_{i}" for i in range(num_hidden_layers)]
        dynamic_axes = {
        ...
        for i in range(num_hidden_layers):
            dynamic_axes[f"past_kv_{i}"] = {0: "batch_size"}
    else:
        input_names = [
            "input_ids",
            "attention_mask",
    ...
    batch_size = 1
    seq_len = 1
    all_len = kv_cache_length if kv_inplace else seq_len + kv_cache_length
    ...
    if perlayer_kv:
        # v7: per-layer KV, each [1, 2*num_kv_heads, kv_len, head_dim]
        per_layer_shape = (1, 2 * num_key_value_heads, kv_cache_length, per_head_dim)
        past_kv_list = tuple(
            torch.rand(per_layer_shape, dtype=torch_dtype).to(device)
            for _ in range(num_hidden_layers)
        )
        input_args = (input_ids, attention_mask, position_ids) + past_kv_list
    elif kv_cache_layout == "BHSD":
    ...
    model.eval()
    with torch.no_grad():
        import modeling_qwen2
        if hasattr(modeling_qwen2, '_KV_INPLACE'):
            modeling_qwen2._KV_INPLACE = kv_inplace
            print(f"[INFO] _KV_INPLACE = {kv_inplace}")
        # v8: fuse QKV weights if model supports it
        if hasattr(model, 'fuse_qkv_weights'):
            model.fuse_qkv_weights()
            print("[INFO] QKV weights fused into qkv_proj")
        # v10: pre-fuse gate_up weights to eliminate Concat in ONNX
        if hasattr(model, 'fuse_gate_up_weights'):
