...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x):
        gate_up = F.linear(x, torch.cat([self.gate_proj.weight, self.up_proj.weight], dim=0))
        gate_out, up_out = gate_up.split(self.intermediate_size, dim=-1)
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # v7: past_key_value is per-layer [1, 2*num_kv_heads, kv_len, head_dim]
        # no Gather from shared 6D tensor — direct access
        ...
        if past_key_value is not None:
            # v7: past_key_value shape: [1, 2*num_kv_heads, kv_len, head_dim]
            # split into K [1, num_kv_heads, kv_len, head_dim] and V [1, num_kv_heads, kv_len, head_dim]
            cache_key = past_key_value[:, :self.num_key_value_heads]
            cache_value = past_key_value[:, self.num_key_value_heads:]
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        *past_kv_list,  # v7: N per-layer KV tensors, each [1, 2*num_kv_heads, kv_len, head_dim]
        ...
        """
        # v7: per-layer KV cache — no reshape needed, each layer gets its own tensor
        # past_kv_list[i] shape: [1, 2*num_kv_heads, kv_len, head_dim]
        past_length = past_kv_list[0].shape[2]
        full_attention_mask = self.get_masks(
            input_ids,
            past_length,
            attention_mask,
        )
        dtype = past_kv_list[0].dtype
        device = input_ids.device
        ...
        for i, decoder_layer in enumerate(self.layers):
            layer_outputs = decoder_layer(
                ...
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=past_kv_list[i],  # v7: per-layer tensor
            )
        ...
        # v7: output per-layer KV as separate tensors (no concat)
        # presents = [K0, V0, K1, V1, ...] → group into per-layer [1, 2*H, seq, D]
        out_kvs = []
        for i in range(0, len(presents), 2):
            out_kvs.append(torch.cat([presents[i], presents[i+1]], dim=1))
        return (
            hidden_states,
            tuple(out_kvs),
        )
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        *past_kv_list,  # v7: N per-layer KV tensors
        # inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        outputs = self.model(
            ...
            attention_mask,
            position_ids,
            *past_kv_list,  # v7: unpack per-layer KV tensors
        ...
        # v7: outputs = (hidden_states, (kv_0, kv_1, ..., kv_27))
        # flatten for ONNX: return (logits, kv_0, kv_1, ..., kv_27)
