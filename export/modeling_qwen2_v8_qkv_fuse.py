...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        # v8: fused QKV projection (will be populated in _fuse_qkv_weights)
        self._qkv_out_dim = self.num_heads * self.head_dim + 2 * self.num_key_value_heads * self.head_dim
        self.qkv_proj = nn.Linear(self.hidden_size, self._qkv_out_dim, bias=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # v8: fused QKV projection — one MatMul instead of three
        qkv = self.qkv_proj(hidden_states)
        query_states = qkv[:, :, :self._q_dim]
        key_states = qkv[:, :, self._q_dim:self._q_dim + self._kv_dim]
        value_states = qkv[:, :, self._q_dim + self._kv_dim:]
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def fuse_qkv_weights(self):
        """v8: Fuse q/k/v_proj weights into qkv_proj for each attention layer."""
        for layer in self.model.layers:
            attn = layer.self_attn
            with torch.no_grad():
                attn.qkv_proj.weight.copy_(torch.cat([
                    attn.q_proj.weight, attn.k_proj.weight, attn.v_proj.weight
                ], dim=0))
                attn.qkv_proj.bias.copy_(torch.cat([
