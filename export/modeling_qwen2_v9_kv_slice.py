    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            # Use narrow (→ ONNX Slice) instead of index (→ ONNX Gather) to avoid
            # reading the entire packed KV tensor for each layer's attention.
            # Slice reads only the contiguous region for this layer (~4MB vs ~110MB).
            cache_kv = past_key_value.narrow(1, self.layer_idx, 1).squeeze(1)  # [1, 2, num_kv_heads, kv_len, head_dim]
            cache_key = cache_kv.narrow(1, 0, 1).squeeze(1)    # [1, num_kv_heads, kv_len, head_dim]
            cache_value = cache_kv.narrow(1, 1, 1).squeeze(1)  # [1, num_kv_heads, kv_len, head_dim]
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        output_cache = (key_states, value_states)
        # past_key_value shape: [1, num_layers, 2, num_kv_heads, kv_len, head_dim]
        # Use narrow (→ ONNX Slice) instead of index (→ ONNX Gather)
        cache_kv = past_key_value.narrow(1, self.layer_idx, 1).squeeze(1)  # [1, 2, num_kv_heads, kv_len, head_dim]
        cache_key = cache_kv.narrow(1, 0, 1).squeeze(1)    # [1, num_kv_heads, kv_len, head_dim]
        cache_value = cache_kv.narrow(1, 1, 1).squeeze(1)  # [1, num_kv_heads, kv_len, head_dim]
        key_states = torch.cat((cache_key, key_states), dim=2)
