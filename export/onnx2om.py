...
parser.add_argument(
    "--kv_cache_layout",
    help="KV cache memory layout: BSHD (default, seq-major) or BHSD (head-major, eliminates Transpose)",
    type=str,
    choices=["BSHD", "BHSD"],
    default="BSHD",
)
parser.add_argument(
    "--kv_inplace",
    help="v11: KV cache in-place update, mask shape = kv_cache_length (no +1 for seq_len)",
    action="store_true",
    default=False,
)
...
try:
    ...
    if args.kv_inplace:
        mask_len_static = kv_cache_length
    else:
        mask_len_static = 1 + kv_cache_length
    attention_mask_shape = [
        f"1~{max_batch}" if max_batch > 1 else "1",
        "-1" if max_prefill_length > 1 else str(mask_len_static)
    ]
    position_ids_shape = [
        f"1~{max_batch}" if max_batch > 1 else "1",
        "-1" if max_prefill_length > 1 else "1"
