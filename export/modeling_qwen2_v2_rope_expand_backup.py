...
# Optimized v1+v2: same logic as original but with static constants
# to avoid Shape/Gather/Div/Mod ops when simplify=false
def rotate_half(x):
    """Rotates half the hidden dims of the input.
    Uses hardcoded half=64 (head_dim=128) to avoid dynamic shape ops.
    """
    return torch.cat((-x[..., 64:], x[..., :64]), dim=-1)


# Copied from transformers.models.llama.modeling_llama.apply_rotary_pos_emb
# Optimized v2: eliminate Expand by using expand_as before multiplication
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    """Applies Rotary Position Embedding to the query and key tensors.
    Optimized: explicit expand_as replaces implicit broadcast (Expand op).
    """
    cos = cos[position_ids].unsqueeze(unsqueeze_dim)
    sin = sin[position_ids].unsqueeze(unsqueeze_dim)
    # Expand cos/sin to match q shape, avoiding implicit broadcast Expand op
    cos_q = cos.expand_as(q)
    sin_q = sin.expand_as(q)
    q_embed = (q * cos_q) + (rotate_half(q) * sin_q)
    # For GQA: k has fewer heads, slice cos/sin accordingly
    cos_k = cos[:, :k.shape[1], :, :].expand_as(k)
    sin_k = sin[:, :k.shape[1], :, :].expand_as(k)
