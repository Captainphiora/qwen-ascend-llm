...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # v7b: past_key_value is per-layer [1, 2*num_kv_heads, kv_len, head_dim]
        ...
        if past_key_value is not None:
            # split K and V from [1, 2*num_kv_heads, kv_len, head_dim]
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        """
        # v7b: Split KV cache into per-layer chunks at graph level
        # input shape: BHSD [1, 112, kv_len, 128]
        # chunk into 28 × [1, 4, kv_len, 128] — generates ONNX Split op (one-time)
        # each layer uses its own chunk directly, no Gather from large tensor
        num_layers = len(self.layers)
        kv_chunks = torch.chunk(past_key_values, num_layers, dim=1)
        # kv_chunks[i]: [1, 4, kv_len, 128] for layer i

        past_length = kv_chunks[0].shape[2]  # kv_len
        ...
        for i, decoder_layer in enumerate(self.layers):
            layer_outputs = decoder_layer(
                ...
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=kv_chunks[i],  # v7b: per-layer chunk
            )
