...
校准方式：完整 prompt prefill（多 token 输入），让模型计算真实的 attention，
收集到的激活分布才能反映实际推理时的数值范围。
...
def main():
    ...
    parser.add_argument("--quant_cfg", type=str, default="",
                        help="Path to simplified quant .cfg file (config_defination)")
    ...
    cfg_defn = args.quant_cfg if args.quant_cfg else None
    if cfg_defn:
    ...
    if cfg_defn:
        print(f"  Custom config_defination: {cfg_defn}")
    ...
    import shutil
    src_dir = os.path.dirname(os.path.abspath(args.model_path))
    dst_dir = os.path.dirname(os.path.abspath(modified_model))
    if src_dir != dst_dir:
        for fname in os.listdir(src_dir):
            ...
            if not os.path.exists(dst_file) and os.path.isfile(src_file):
                ...
                print(f"  Copied external data: {fname}")
    ...
    for idx, prompt in enumerate(prompts):
        ...
        seq_len = min(len(tokens), args.kv_cache_length - 1)
        ...
        print(f"  Sample {idx+1}/{len(prompts)} done (seq_len={seq_len}, {elapsed:.1f}s)")
