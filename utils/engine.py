...
class ACLModel:
    def __init__(self, config: InferenceConfig, context=None,callback=None):
        ...
        self.perlayer_kv = config.perlayer_kv
        self.kv_inplace = config.kv_inplace
    ...
    def get_inputs(self, seq_len: int) -> List[np.ndarray]:
        ...
        if self.kv_inplace:
            mask = np.ones((1, temp_kv_size), dtype=np.int64)
            mask[:, self.real_kv_size + seq_len : temp_kv_size] = 0
        else:
    ...
    def reset(self):
        ...
        if self.perlayer_kv:
            # v7: zero out all per-layer KV buffers (inputs[3] through inputs[3+N-1])
            for l in range(self.num_layers):
                idx = 3 + l
                ret = acl.rt.memset(
                    self.inputs[idx]["buffer"],
                    self.inputs[idx]["size"],
                    0,
                    self.inputs[idx]["size"]
                )
                check_ret(f"reset device kv-cache layer {l}", ret)
        else:
            ret = acl.rt.memset(
                self.inputs[3]["buffer"],
                self.inputs[3]["size"],
                0,
                self.inputs[3]["size"]
            )
    ...
    def update_kv_cache(self, seq_len):
        ...
        if self.perlayer_kv:
            # v7: per-layer KV cache update
            # Each output[1+l] is [1, 2*num_kv_heads, seq_len, head_dim] for layer l
            # Each input[3+l] is [1, 2*num_kv_heads, kv_cache_length, head_dim]
            n_kv_heads = 2 * self.config.num_key_value_heads
            ...
            for l in range(self.num_layers):
                ...
                for h in range(n_kv_heads):
                    ...
                    ret = acl.rt.memcpy(
                        self.inputs[in_idx]["buffer"] + dst_off,
                        chunk,
                        self.outputs[out_idx]["buffer"] + src_off,
                        chunk,
                        ACL_MEMCPY_DEVICE_TO_DEVICE
                    )
                    check_ret(f"update kv layer {l} head {h}", ret)
        elif self.kv_cache_layout == "BHSD":
    ...
    def inference(self, input_data_list: List[np.ndarray], seq_length=1, is_dynamic=False, is_prefill=False) -> List[np.ndarray]:
        ...
        if is_dynamic:
            ...
            if (self.real_kv_size + seq_length) > self.kv_cache_length // 2:
                dynamic_dims = [
                    self.max_batch, seq_length,
                    self.max_batch, seq_length + self.kv_cache_length,
                    self.max_batch, seq_length
                ]
                if self.perlayer_kv:
                    for _ in range(self.num_layers):
                        dynamic_dims += list(self.past_key_value_shape)
                else:
                    dynamic_dims += list(self.past_key_value_shape)
            else:
                dynamic_dims = [
                    self.max_batch, seq_length,
                    self.max_batch, seq_length + self.kv_cache_length // 2,
                    self.max_batch, seq_length
                ]
                if self.perlayer_kv:
                    half_shape = list(self.past_key_value_shape)
                    half_shape[2] = half_shape[2] // 2
                    for _ in range(self.num_layers):
                        dynamic_dims += half_shape
                else:
