...
class InferenceConfig:
    def __init__(
        ...
        max_prefill_length: int = 1,
        kvcache_method: str = "fixsize",
        kv_cache_length: int = 2048,
        cache_format: str = 'huggingface-tensor',
        dtype: str = "float16",
        ...
        self.perlayer_kv = perlayer_kv
        self.kv_inplace = kv_inplace
        if perlayer_kv:
            # v7: per-layer KV cache, each [1, 2*num_kv_heads, kv_len, head_dim]
            self.per_layer_kv_shape = (
                self.max_batch,
                2 * self.num_key_value_heads,
                self.kv_cache_length,
                self.per_head_dim
            )
            self.past_key_value_shape = self.per_layer_kv_shape
