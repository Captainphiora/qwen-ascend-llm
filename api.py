...
show_progress = True
MODEL_NAME = config.hf_model_dir.rstrip("/").split("/")[-1]


@app.get("/")
async def root():
    return {f"message": f"{MODEL_NAME} API is running", "status": "ok"}
@app.post("/v1/clear")
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if request.top_p is not None:
        sampling_config["sampling_value"] = request.top_p
    if request.temperature is not None:
        sampling_config["temperature"] = request.temperature
    if request.do_sample is not None and not request.do_sample:
        sampling_config["sampling_method"] = "greedy"
    ...
    print(f"  do_sample    = {request.do_sample}")
    print(f"  stream       = {request.stream}")
    print(f"  system_prompt  = {system}")
    print(f"  stop_words     = {request.stop}")
    print(f"[DEBUG] Effective sampling_config: {sampling_config}")
    print(f"[DEBUG] Effective max_new_tokens:  {max_new_tokens}")
    print("=" * 50)
    ...
    if request.functions:
        ...
        prompt_tokens = len(infer_engine.tokenizer.encode(query if query is not _TEXT_COMPLETION_CMD else ""))
        completion_tokens = len(infer_engine.tokenizer.encode(response))
        usage = UsageInfo(
